...
class KD_Model(nn.Module):
    ...
    def __init__(
        ...
        if Path(model).suffix in {".yaml", ".yml"}:
            self._new(model, task=task, verbose=verbose, teacher_model=self.teacher_model, distillation_alpha=self.distillation_alpha, temperature=self.temperature,feature_layers=self.feature_layers)
        else:
    ...
    def _new(self, cfg: str, task=None, model=None, verbose=False,teacher_model=None, distillation_alpha=0.5, temperature=3,feature_layers=None) -> None:
        """
        ...
        self.model = (model or self._smart_load("model"))(cfg_dict, verbose=verbose and RANK == -1,teacher_model=teacher_model, distillation_alpha=distillation_alpha, temperature=temperature,feature_layers=feature_layers)  # build model
